...
var emailEl = form.querySelector('input[name="email"]');
var email = (emailEl && emailEl.value || '').trim();
if(!zip || !phone || !email) return false;
var lang = (document.documentElement.lang || '').slice(0,2) || (location.pathname.indexOf('/en/') === 0 ? 'en' : 'fi');
var payload = { service: service, zip: zip, phone: phone, email: email, customer_email: email, send_to_email: email, source: source || 'footer-cta', page: window.location.href, lang: lang };
fetch('/.netlify/functions/submit-offer-lead', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) }).catch(function(){});
var params = new URLSearchParams();
...
if(service) params.append('service', service);
params.append('source', source || 'footer-cta');
params.append('page', window.location.href);
...
if (data.total) {
document.querySelectorAll('.rating-count').forEach(function(el){ el.textContent = data.total + ' reviews'; });
}
